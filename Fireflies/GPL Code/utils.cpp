...
void clamp_vec(Vec3& vec, double max)
{
    for (int i = 0; i < 3; i++) {
...
rgbColor hsv_to_rgb(const hsvColor& hsv)
{
    ...
    // Mac OS X Version: removed the call to floor which was causing a crash on Mac OS X 10.1
...
{
    std::deque<Event>::iterator it = events.begin();
    while (it != events.end()) {
	if (when < (*it).second)
	    break;
	it++;
...
#ifdef DEBUG
	    std::cerr << val << " changed to " << newprob << " out of " <<
		max_prob << std::endl;
#endif
	    return;
	}
