...
public:
    ...
    std::vector<Tail*> dead_tails;

    Control camera;	// camera orientation
    double curtime;	// total time the program's been running
    Vec3 wind;		// current wind direction
    Vec3 accel;		// wind is changing
    ...
    Vec3 matrix_axis;	// the axis to rotate around matrix-style
    ...
    void add_flies(unsigned long n);
    // remove 'n' flies from random baits
    void rem_flies(unsigned long n);
    // resize the scene.
