...
void Tail::draw(Scene * inScene)
{
    if (links.size() < 2) // need at least 2 links
	return;

    std::deque<Link>::iterator it = links.begin();
    double glow_width = inScene->glow_factor*inScene->tail_width;
    double stretch_factor = 2*inScene->fsize*inScene->wind[0];
    double dx1, dx2;

    dx2 = ((*it).glow ? glow_width : inScene->tail_width);
    for (; (it+1) != links.end(); it++) {
	// half-width of the tail
	dx1 = dx2;
	dx2 = ((*(it+1)).glow ? glow_width : inScene->tail_width);

	// have the wind stretch the tail (greater effect on ends)
	double age = (*it).age/inScene->tail_length;
	double stretch = stretch_factor*age*age;
	double alpha = 0.9 - age;
	if (alpha > inScene->tail_opaq)
	    alpha = inScene->tail_opaq;
...
bool Tail::elapse(Scene * inScene,double t)
{
    ...
    while (!links.empty() && links.back().age >= inScene->tail_length)
	links.pop_back();
