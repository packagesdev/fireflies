...
Scene::~Scene()
{
    ...
    {
        flies[i]->tail->owner = 0;
        dead_tails.push_back(flies[i]->tail);
...
void Scene::create()
{
    ...
    scene_start_mode(this,-1); // non-existent, just to initialize
...
void Scene::add_flies(unsigned long n)
{
    ...
    long groupsize = (flies.size()+n)/(3*baits.size());
...
void Scene::rem_flies(unsigned long n)
{
    ...
    std::vector<Firefly*>::iterator it = flies.begin();
    while (it != flies.end() && n > 0) {
	if ((*it)->bait == b)
    {
        (*it)->tail->owner = 0;
        dead_tails.push_back((*it)->tail);
...
void Scene::draw()
{
    ...
    std::vector<Tail*>::iterator it = dead_tails.begin();
...
void Scene::elapse_once(double t)
{
	...
	matrix += t;
	if (matrix >= mode_when)
	    scene_start_mode(this,-SMODE_MATRIX);
	...
	scene_start_mode(this,mode_next);
    ...
    // elapse, my children
    for (GLuint i = 0; i < baits.size(); i++)
	baits[i]->elapse(this,t);

    for (GLuint i = 0; i < flies.size(); i++)
	flies[i]->elapse(this,t);

    std::vector<Tail*>::iterator it = dead_tails.begin();
    while (it != dead_tails.end()) {
	if ((*it)->elapse(this,t)) { // he's dead!
	    delete (*it);
